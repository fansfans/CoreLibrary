...
public class TagCloudView extends ViewGroup{
    ...
    private ColorStateList mTagColorStateList;
...
//    private boolean mShowRightImage;
    ...
    private static final int DEFAULT_TEXT_BACKGROUND = R.drawable.selector_tag_background;
...
//    private static final int DEFAULT_RIGHT_IMAGE = R.drawable.arrow_right;
    ...
    public TagCloudView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        mTagSize = a.getFloat(R.styleable.TagCloudView_tcvTextSize, DEFAULT_TEXT_SIZE);
...
//        }
        ...
        if (mShowEndText) {
            ...
            if (mTagResId == DEFAULT_TAG_RESID) {
                ...
                if (mTagColorStateList != null) {
                    endText.setTextColor(mTagColorStateList);
                }
            }
            @SuppressLint("DrawAllocation")
    ...
    public void setTagColor(ColorStateList color) {
        this.mTagColorStateList = color;
    }
        ...
        if (tags != null && tags.size() > 0) {
            for (int i = 0; i < tags.size(); i++) {
                ...
                if (mTagResId == DEFAULT_TAG_RESID) {
                    ...
                    if (mTagColorStateList != null) {
                        tagView.setTextColor(mTagColorStateList);
                    }
                }
                LayoutParams layoutParams = new LayoutParams(LayoutParams.WRAP_CONTENT, LayoutParams.WRAP_CONTENT);
    ...
    public void setTagsSelected(boolean selected, int ... position) {
        int len = position.length;
        int length = Math.min(getChildCount(), len);

        for (int i = 0; i < length; i++) {
            getChildAt(position[i]).setSelected(selected);
        }
        postInvalidate();
    }
