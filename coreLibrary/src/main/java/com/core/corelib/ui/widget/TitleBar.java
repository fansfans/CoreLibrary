...
public class TitleBar extends ViewGroup implements View.OnClickListener {
    ...
    private static final int SINGALE_LEFT_OR_RIGHT_DISTANCE_TO_SCREEN = 16; // dip
    ...
    private int mOutPaddingTOFrame;
    ...
    private void init(Context context) {
        ...
        mOutPaddingTOFrame = dip2px(SINGALE_LEFT_OR_RIGHT_DISTANCE_TO_SCREEN);
...
//        mLeftText.setBackgroundColor(context.getResources().getColor(R.color.blue));
//        mCenterLayout.setBackgroundColor(context.getResources().getColor(R.color.red_middle));
//        mRightLayout.setBackgroundColor(context.getResources().getColor(R.color.green));
...
//        mRightLayout.setPadding(mOutPadding, 0, mOutPadding, 0);
        mRightLayout.setPadding(mOutPaddingTOFrame, 0, mOutPaddingTOFrame, 0);
    ...
    public void setActionTextColor(int color, int index) {
    ...
    public void setActionTextColor(ColorStateList colorStateList, int index) {
    ...
    public void setActionEnable(boolean enable) {
        for (int i = 0; i < mRightLayout.getChildCount(); i++) {
            mRightLayout.getChildAt(i).setEnabled(enable);
        }
    }

    public void setActionEnable(boolean enable, int index) {
        if (index < -1 || index >= getChildCount()) {
            return;
        }
        mRightLayout.getChildAt(index).setEnabled(enable);
    }
    ...
    public void setActionTextSize(float size) {
        ...
        for (int i = 0; i <= mRightLayout.getChildCount(); i++) {
            View view = mRightLayout.getChildAt(i);
            if (view instanceof TextView) {
                ((TextView)view).setTextSize(mActionTextSize);
            }
        }
    }

    public void setActionTextSize(float size, int index) {
        mActionTextSize = size;
    ...
    private View inflateAction(Action action) {
        ...
        if (TextUtils.isEmpty(action.getText())) {
            ...
            if (!(Math.abs(mActionTextSize - 0.0) <= 0.00000001F)) {
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        boolean isLeftPositionChanged = false;
        if (drawables != null && drawables.length > 0 && drawables[0] != null && TextUtils.isEmpty(mLeftText.getText())) {
            isLeftPositionChanged = true;
            mLeftText.layout(mOutPaddingTOFrame, mStatusBarHeight, mOutPaddingTOFrame + mLeftText.getMeasuredWidth(), mLeftText.getMeasuredHeight() + mStatusBarHeight);

        } else if (!TextUtils.isEmpty(mLeftText.getText()) && (drawables == null || drawables.length == 0 || drawables[0] == null)) {
            isLeftPositionChanged = true;
            mLeftText.layout(mOutPaddingTOFrame, mStatusBarHeight, mOutPaddingTOFrame + mLeftText.getMeasuredWidth(), mLeftText.getMeasuredHeight() + mStatusBarHeight);
        ...
        if (!isLeftPositionChanged) {
            ...
            if (mOutPaddingTOFrame + mLeftText.getMeasuredWidth() > mRightLayout.getMeasuredWidth()) {
                mCenterLayout.layout(mOutPaddingTOFrame + mLeftText.getMeasuredWidth(), mStatusBarHeight,
                        mScreenWidth - mLeftText.getMeasuredWidth()- mOutPaddingTOFrame, getMeasuredHeight());
            } else {
                mCenterLayout.layout(mRightLayout.getMeasuredWidth(), mStatusBarHeight,
                        mScreenWidth - mRightLayout.getMeasuredWidth(), getMeasuredHeight());
            }
