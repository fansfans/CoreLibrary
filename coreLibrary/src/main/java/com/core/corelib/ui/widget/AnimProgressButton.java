...
public class AnimProgressButton extends TextView {
    ...
    private void initAttrs(Context context, AttributeSet attrs) {
        ...
        mBackgroundColor = a.getColor(R.styleable.AnimProgressButton_progressbtn_backgroud_color, Color.parseColor("#6699ff"));
        mBackgroundSecondColor = a.getColor(R.styleable.AnimProgressButton_progressbtn_backgroud_second_color, Color.LTGRAY);
        mButtonRadius = a.getFloat(R.styleable.AnimProgressButton_progressbtn_radius, getMeasuredHeight() / 2);
        mTextColor = a.getColor(R.styleable.AnimProgressButton_progressbtn_text_color, mBackgroundColor);
    ...
    public void setState(int state) {
        if (mState != state) {//状态确实有改变
            this.mState = state;
            invalidate();
            if (state == AnimProgressButton.INSTALLING) {
                //开启两个点动画
    ...
    public void setProgressText(String text, float progress) {
        if (progress >= mMinProgress && progress <= mMaxProgress) {
            ...
            = text + String.format("%s %%", (int)progress);
