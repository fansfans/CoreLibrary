package com.core.corelib.ui.recycler;
...
import com.core.corelib.ui.recycler.listener.OnLoadMoreListener;
import com.core.corelib.ui.recycler.listener.OnRefreshListener;
import com.core.corelib.ui.recycler.util.FloatUtil;

public class SuperRecyclerView extends FrameLayout {

    protected int ITEM_LEFT_TO_LOAD_MORE = 4;
    ...
    protected void initAttrs(AttributeSet attrs) {
        ...
        try {
            mSuperRecyclerViewMainLayout = a.getResourceId(R.styleable.superrecyclerview_mainLayoutId, R.layout.lib_recycler_layout_super_recyclerview);
            ...
            mEmptyId = a.getResourceId(R.styleable.superrecyclerview_layout_empty, R.layout.lib_recycler_layout_load_empty);
            mMoreProgressId = a.getResourceId(R.styleable.superrecyclerview_layout_moreProgress, R.layout.lib_recycler_layout_load_more_progress);
            mProgressId = a.getResourceId(R.styleable.superrecyclerview_layout_progress, R.layout.lib_recycler_layout_load_progress);
        } finally {
            a.recycle();
        }
    }
    ...
    private void initView() {
        ...
        mMoreProgress = (ViewStub) v.findViewById(R.id.load_more_progress);
...
//        Log.d("TAG", "totalItemCount: "+ totalItemCount + " visibleItemCount: "+ visibleItemCount + " lastVisibleItemPosition: "+ lastVisibleItemPosition);

        if ((((totalItemCount - lastVisibleItemPosition) <= ITEM_LEFT_TO_LOAD_MORE && totalItemCount > visibleItemCount) ||
             (totalItemCount - lastVisibleItemPosition) == 0 && totalItemCount > visibleItemCount)
            && !isLoadingMore && mLoadingMoreEnable) {

            isLoadingMore = true;
            if (mOnLoadMoreListener != null) {
                mMoreProgress.setVisibility(View.VISIBLE);
                mOnLoadMoreListener.onMoreAsked(mRecycler.getAdapter().getItemCount(), ITEM_LEFT_TO_LOAD_MORE, lastVisibleItemPosition);
            }
        }
    }
...
//    }
    ...
    public void setCenterLoading(boolean loading) {
        if (loading) {
     ...
     */
    public void setOnLoadMoreListener(OnLoadMoreListener onMoreListener, int max) {
    ...
    /**
     * show/hide the More event ui
     */
    public void setLoading(boolean isLoading) {
        this.isLoadingMore = isLoading;
        if (isLoading) {
            mMoreProgress.setVisibility(VISIBLE);
            mEmpty.setVisibility(GONE);
        } else {
            mProgress.setVisibility(GONE);
        }
    ...
    public void setLoadingMore(boolean enable) {
        ...
        this.mLoadingMoreEnable = enable;
    }
    ...
    public void setCenterEmpty(boolean visible) {
        if (visible) {
